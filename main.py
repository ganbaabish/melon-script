...
# def insert_error_log(error:str)->None:
#     data = {"error_date": datetime.datetime.now(), "error":error}
#     requests.post(url+"/api/insert_logs", json=data)


def run(token_keys: dict):

    client = tweepy.Client(
                            bearer_token = token_keys['bearer_token'], 
                            consumer_key = token_keys['consumer_key'],
                            consumer_secret = token_keys['consumer_secret'],
                            access_token = token_keys['access_token'],
                            access_token_secret = token_keys['access_token_secret']
                            )
    ...
    for users in tweepy.Paginator(client.get_users_following, target_id, user_fields=['public_metrics'], max_results=1000, limit=10):
        ...
        for user in users.data:
            ...
            tweets = client.get_users_tweets(
                                ...
                                start_time = start_time()                           #tweet's created date must be within 24 hours
                                )
    ...
    for line in data:
        ...
        if temp in words:
            ...
            if check:
                ...
                arr.append(line)
    data = sorted(arr, key=take_key, reverse=True)
    all_trending_tweets = []
    ...
    for i in range(10):
        ...
        try:
            client = tweepy.Client(
                        bearer_token=elevated_tokens['bearer_token'][0], 
                        consumer_key=elevated_tokens['consumer_key'][0],
                        consumer_secret=elevated_tokens['consumer_secret'][0], 
                        access_token=elevated_tokens['access_token'][0], 
                        access_token_secret=elevated_tokens['access_token_secret'][0]
                        )
            ...
            try:
                client = tweepy.Client(
                            bearer_token=elevated_tokens['bearer_token'][1], 
                            consumer_key=elevated_tokens['consumer_key'][1],
                            consumer_secret=elevated_tokens['consumer_secret'][1], 
                            access_token=elevated_tokens['access_token'][1], 
                            access_token_secret=elevated_tokens['access_token_secret'][1]
                            )
                num_of_tweets = client.get_recent_tweets_count(query=word).meta["total_tweet_count"]
                ...
                client = tweepy.Client(
                            bearer_token=elevated_tokens['bearer_token'][2], 
                            consumer_key=elevated_tokens['consumer_key'][2],
                            consumer_secret=elevated_tokens['consumer_secret'][2], 
                            access_token=elevated_tokens['access_token'][2], 
                            access_token_secret=elevated_tokens['access_token_secret'][2]
                            )
                num_of_tweets = client.get_recent_tweets_count(query=word).meta["total_tweet_count"]
                tweets = client.search_recent_tweets(
                            query=word, 
    ...
    request_for_trending_tweets = requests.post(url+"/api/trending_tweets", json=all_trending_tweets)

    # print("Trending tweets results:")
    # print(request_for_trending_tweets.json())
    ...
    tweet_rank_data = {"created_at": start_time()}
    ...
    for tweet in tweet_rank.json():
        if 'RT' not in tweet["tweet_text"]:
            ...
            tweets.append(data)
    tweets = sorted(tweets, key=take_score, reverse=True)
    ...
    for i in range(10):
        try:
            ...
            print("Summary", i, "tweet rank inserted")
    print("Done...!")
    print("Now", datetime.datetime.now())
    print("Sleeping...!")
    time.sleep(1200)
